...
for i, invp in enumerate(invp_):
    ...
    for j, r in enumerate(r_):
        ...
        mu_, sigma_, sharpe_, mdd_, sterling_ = [], [], [], [], []
        for _ in range(len(all_prices) - 365):
            prices = all_prices[_:(_+365)]
            ...
            mu_.append(mu)
            sigma_.append(sigma)
            sharpe_.append(sharpe)
            mdd_.append(mdd)
            sterling_.append(sterling)

        results['mu'][j, i] = np.mean(mu_)
        results['sigma'][j, i] = np.mean(sigma_)
        results['sharpe'][j, i] = np.mean(sharpe_)
        results['mdd'][j, i] = np.mean(mdd_)
        results['sterling'][j, i] = np.mean(sterling_)
